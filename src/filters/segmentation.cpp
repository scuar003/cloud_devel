...
class Segmentation : public rclcpp::Node {
    ...
    Segmentation () : Node ("Segmentation"), tf_buffer(this->get_clock()), tf_listener(tf_buffer){ 
        cloud_sub = create_subscription<Pointcloud2>("/camera/depth/color/points", 10, std::bind(&Segmentation::cloudProcessing, this, _1));
        filtered_cloud_pub = create_publisher<Pointcloud2>("Segmentation", 10);
    ...
    private:
        ...
        void publishFilteredCloud(const PCLcloud::Ptr &cloud) {
            ...
            for (size_t i = 0; i < ros2_cloud.width * ros2_cloud.height; ++i) {
                ...
                ros2_cloud.data[offset + 2] = 255;   // B
            }
