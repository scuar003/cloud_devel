...
class Downsample : public rclcpp::Node {
    public:
        Downsample () : Node ("Downsample_Cloud"), tf_buffer(this->get_clock()), tf_listener(tf_buffer){ 
            cloud_sub = create_subscription<Pointcloud2>("/camera/depth/color/points", 10, std::bind(&Downsample::cloudProcessing, this, _1));
            filtered_cloud_pub = create_publisher<Pointcloud2>("downsample", 10);
    ...
    private:
        ...
        void applyFilter(const PCLcloud::Ptr &cur_cloud, PCLcloud::Ptr &filter_cloud) {
            ...
            voxel_grid.setInputCloud(cur_cloud);
            voxel_grid.setLeafSize(leaf_size, leaf_size, leaf_size);
            voxel_grid.filter(*filter_cloud);
        ...
        void publishFilteredCloud(const PCLcloud::Ptr &cloud) {
            ...
            for (size_t i = 0; i < ros2_cloud.width * ros2_cloud.height; ++i) {
                ...
                ros2_cloud.data[offset] = 0;   // R
                ros2_cloud.data[offset + 1] = 255; // G
...
int main (int argc, char** argv) {
    ...
    auto node = std::make_shared<Downsample>();
    rclcpp::spin(node);
